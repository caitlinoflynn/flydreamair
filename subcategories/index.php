...
include(ROOT_PATH . "/app/controllers/subcategories.php");

if (isset($_GET['s_id'])) {
    $subcategory = selectOne('flydreamair_subcategories', ['id' => $_GET['s_id']]);
    $category = selectOne('flydreamair_categories', ['id' => $subcategory['category_id']]);
...
$subcategoryLists = getSubcategoriesByCategory();
...
<div class="wrap">
  <div class="links">
    <a href="<?php echo FDA_BASE_URL . '/' ?>"><span>Home</span><span> / </span></a><a href="<?php echo FDA_BASE_URL . '/' . $category['id'] . '/' . $category['name'] ?>"><span><?php echo $category['name'] ?><span> / </span></a>
    <span><?php echo $subcategory['name'] ?></span>
  </div>
  <div class="container">
    <h1 class="heading"><?php echo $subcategory['name'] ?></h1>
    ...
    <div class="results-bar mobile">
      ...
      <div class="filter-bar">
        <ul>
          ...
          <?php foreach($subcategoryLists as $key => $subcategoryList): ?>
          <?php if($subcategoryList['name'] == $subcategory['name']): ?>
          <a href="<?php echo FDA_BASE_URL . '/subcategory/' . $subcategoryList['id'] . '/' . $subcategoryList['handle'] ?>">
              <li style="color: #292129">
            <?php echo $subcategoryList['name'] ?>
          </li>
          </a>
          <?php else: ?>
          <a href="<?php echo FDA_BASE_URL . '/subcategory/' . $subcategoryList['id'] . '/' . $subcategoryList['handle'] ?>">
